...
public class SmartsheetModelDeserializationTest {
	...
	public void deserializesSmartsheetUsers() throws JsonMappingException, JsonParseException, IOException {
		JsonDeserializer<SmartsheetPagingwrapper<SmartsheetUser>> deserializer = new JsonDeserializer<SmartsheetPagingwrapper<SmartsheetUser>>();
		String json = TestUtils.getSampleGetUsersJsonResponse();
		SmartsheetPagingwrapper<SmartsheetUser> usersPagingWrapper = deserializer.deserialize(json,
				new TypeReference<SmartsheetPagingwrapper<SmartsheetUser>>() {
				});
		List<SmartsheetUser> users = usersPagingWrapper.getData();
		assertNotNull(users);
